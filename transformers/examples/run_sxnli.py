...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset, random_init_result, n_points  = load_and_cache_examples(args, eval_task, args.language, tokenizer, evaluate=True)
        ...
        if args.is_baseline_run:
            result.update({'random_init_result': random_init_result})
            scores = []
            for _ in range(args.num_bags):
                bag_idxs = np.random.choice(len(preds), len(preds))
                score_dict = compute_metrics(eval_task, preds[bag_idxs], out_label_ids[bag_idxs])
                scores.append(score_dict[sorted(score_dict.keys())[0]])
            result.update({'tol': np.std(scores)})
            result.update({'mean_score': np.mean(scores)})
...
def load_and_cache_examples(args, task, train_language, tokenizer, evaluate=False):
    if args.local_rank not in [-1, 0] and not evaluate:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    args.train_language = train_language
    processor = processors[task](language=args.language, train_language=train_language)
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        logger.info("-"*120)
        logger.info("Creating features from dataset file as %s", cached_features_file)
        logger.info("-" * 120)
    ...
    # Data Shepley debug
    if args.data_size and not evaluate:
...
def main():
    ...
    parser.add_argument(
        "--data_size", default=None, type=int,
        help="Data_size for Shapley Training (None for full data), e.g., 100 for debug ",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--is_baseline_run", action="store_true",
        help="Is the baseline run to get result, data_size, random/init score",
    )
    parser.add_argument(
    ...
    if args.do_train:
        ...
        ALL_BINARY_TASKS.remove(args.language)

        logger.info(" ALL_BINARY_TASKS = %s, target language = %s args.indices_to_delete_file_path = %s", ALL_BINARY_TASKS,
                    args.language, args.indices_to_delete_file_path)
        logger.info(" not evaluate = %s args.indices_to_delete_file_path and not evaluate=%s", not evaluate,
                    args.indices_to_delete_file_path and not evaluate)
        if args.indices_to_delete_file_path:
            with open(args.indices_to_delete_file_path, "r") as reader:
                ...
                logger.info(" After delete ALL_train_languages = %s", ALL_BINARY_TASKS)
        ...
        if len(ALL_BINARY_TASKS) > 0:
            train_dataset, random_init_result, n_train_points = \
                load_and_cache_examples(args, args.task_name, ALL_BINARY_TASKS[0], tokenizer, evaluate=False)

            for train_language in ALL_BINARY_TASKS[1:]:
                train_dataset2, random_init_result2, n_train_points2 = \
                    load_and_cache_examples(args, args.task_name,  train_language, tokenizer, evaluate=False)
