...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    eval_dataset, random_init_result, n_points, = load_and_cache_examples(args, args.task_name, tokenizer, labels, pad_token_label_id, mode=mode)
    ...
    try:
        ...
        if args.data_size and mode == "train":
            if args.data_size < len(features):
                logger.info('-' * 50)
                logger.info(f'originial data size: {len(features)} truncated to {args.data_size}')
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The name of the task to train selected in the ud2: " ,
    )
    parser.add_argument(
        "--base_data_dir",
        default=None,
    ...
    args.base_data_dir = args.data_dir
    ...
    if args.do_train:
        ...
        logger.info(" ALL_POS_TASKS = %s, task = %s args.indices_to_delete_file_path = %s", ALL_BINARY_TASKS,
                    args.task_name, args.indices_to_delete_file_path)
        ...
        if len(ALL_BINARY_TASKS) > 0:
            train_dataset, random_init_result, n_train_points = \
                load_and_cache_examples(args, ALL_BINARY_TASKS[0], tokenizer, labels, pad_token_label_id, mode="train")


        for task in ALL_BINARY_TASKS[1:]:
            train_dataset2, random_init_result2, n_train_points2 = \
                load_and_cache_examples(args, task, tokenizer, labels, pad_token_label_id, mode="train")
