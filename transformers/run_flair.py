...
def get_baseline_results(sources, target_domain, train_data_size=None):
    ...
    for domain in sources:
        temp_data = random.choices(ALL_POS_ALL_CORPUS[domain]._train, k=train_data_size) \
            if len(ALL_POS_ALL_CORPUS[domain].train)>train_data_size and train_data_size else ALL_POS_ALL_CORPUS[domain]._train
    ...
    result_dict = trainer.train(train_output_dir, train_with_dev = True, \
                           embeddings_storage_mode = 'cpu', mini_batch_size = 256, \
                           max_epochs = num_train_epochs)
    return result_dict



sources = ALL_POS_DOMAINS
