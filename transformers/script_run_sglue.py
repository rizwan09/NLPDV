...
tmc_run = True
g_run = False
load_removing_performance_plot = load_adding_performance_plot = False
...
n_points_file = os.path.join(train_output_dir, "training_results"+str(seed)+".txt")
...
if os.path.exists(loo_dir):
    print(f'oading {loo_dir}', flush=True)
...
if not n_points:
    ...
    command = eval_run_command + ' --is_baseline_run --seed ' + str(seed) + ' ' + ' --data_size '+str(eval_data_size)
    print(command, flush=True)
    os.system(command)
...
if loo_run:
    try:
        len(vals_loo)
        if len(vals_loo) != train_data_size:
            vals_loo = None
        else:
            loo_run = False
    except:
...
def run_routine(tmc_run, train_run_command, eval_run_command, eval_data_size, num_train_epochs, eval_output_dir,  n_points_file, mem_tmc, idxs_tmc, random_score, mean_score, n_points, save_every, seed, max_iter, tolerance, sources):
    for iter_counter in trange(max_iter, desc='Overall Shpley Value Calculation'):
        ...
        if tmc_run:
            # pdb.set_trace()
            error_ = error(mem_tmc, min_convergence_iter=50)
            print('=' * 50, flush=True)
            ...
            if error_ < err:
                ...
                mem_tmc, idxs_tmc = _tmc_shap(train_run_command + ' --num_train_epochs ' + str(num_train_epochs), \
                                              eval_run_command + ' --data_size ' + str(eval_data_size), eval_output_dir, \
                ...
                assert mem_tmc.shape[0] - old_shape == save_every
...
performance_plots_adding(directory, [vals_tmc, vals_loo], train_task_name, eval_task_name,\
                         ...
                         num_plot_markers=20, sources=sources, rnd_iters=2, length=None)
