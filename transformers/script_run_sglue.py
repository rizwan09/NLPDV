...
model_type = 'bert'
train_model_name_or_path= 'bert-large-uncased-whole-word-masking'
do_lower_case = True
...
# Seed
seed = 42
max_iter = cluster_num = train_data_size//clsuter_size
save_every = 5
...
# Shapley params
metric = 'acc'
sources = [i//clsuter_size for i in range(train_data_size)]
loo_run = True
tmc_run = True
g_run = False
overwrite_shapley = True
load_shapley = False # True when just load shapley's when to plot only


train_output_dir = 'temp/'+train_task_name+'_output/'
eval_output_dir = 'temp/'+eval_task_name+'_output/'
...
if overwrite_shapley: load_shapley = False
if load_shapley: tmc_run = False
train_data_dir = GLUE_DIR+'/'+train_task_name+'/'
eval_data_dir = GLUE_DIR+'/'+eval_task_name+'/'
...
run_command += ' ' + run_file + ' ' + ' --model_type ' + model_type + \
               ...
               +' --num_train_epochs '+str(num_train_epochs)+' --overwrite_output_dir '
...
def _calculate_loo_vals(sources, baseline_value, n_points, eval_output_dir, train_run_command, eval_run_command, n_points_file):
    ...
    for i in tqdm(sources.keys()):
        ...
        assert n_points2 == n_points - len(sources[i])
...
def error(mem, min_convergence_iter = 100):
...
def performance_plots(directory, vals, train_task_name, eval_task_name, train_run_command, eval_run_command,\
    ...
    removing_performance_dir = os.path.join(directory, train_task_name+'_'+eval_task_name+'_'+str(n_points)+'_removing_performance.pkl')
    if os.path.exists(removing_performance_dir) and load_shapley:
        ...
        if not length:
            ...
            for vals_source in tqdm(vals_sources, desc='Ploting Removal LOO/Shapley: '):
                ...
                perfs.append(_portion_performance( np.argsort(vals_source)[::-1], \
                                      plot_points, train_run_command, eval_run_command, random_score, n_points, sources=sources))
            ...
            for itr in trange(rnd_iters, desc='Ploting Removal Random: '):
                ...
                rnd.append(_portion_performance( np.random.permutation( \
                    ...
                    train_run_command, eval_run_command, random_score, n_points, sources=sources) )
...
def _portion_performance(idxs, plot_points, train_run_command, eval_run_command, random_score, n_points, sources=None):
    ...
    for i in trange(len(plot_points), 0, -1, desc='Inside LOO/Shapley: removing sources in descending'):
...
def performance_plots_adding(directory, vals, train_task_name, eval_task_name, train_run_command, eval_run_command, random_score, n_points, name=None, num_plot_markers=20,
                             sources=None, rnd_iters=2, length=None):
    """Plots the effect of removing valuable points.
    ...
    adding_performance_dir = os.path.join(directory, train_task_name + '_' + eval_task_name + '_' + str(
        n_points) + '_adding_performance.pkl')
    if os.path.exists(adding_performance_dir) and load_shapley:
        ...
        if not length:
            ...
            for itr in trange(rnd_itersdesc='Ploting Adding Random: '):
                print("="*60, flush=True)
                print(f"Calulating performace if we remove points in random iter {itr}", flush=True)
                print("=" * 60, flush=True)
    ...
    if directory is not None and name is not None:
        plt.savefig(os.path.join(
            directory, 'plots', '{}.png'.format(name + '_adding_')),
            bbox_inches='tight')
        plt.close()


def _portion_performance_addition(idxs, plot_points, train_run_command, eval_run_command, random_score, n_points, sources=None):
        ...
        scores = []
        all_ids = np.array(range(n_points))
        for i in trange(0, len(plot_points), 1, desc='Inside LOO/Shapley: adding sources in descending'):
...
if overwrite_shapley and os.path.exists(directory):
...
if not n_points:
    ...
    command = train_run_command + ' --is_baseline_run --seed ' + str(seed) + ' '
    print(command, flush=True)
    os.system(command)
    ...
    command = eval_run_command + ' --is_baseline_run --seed ' + str(seed) + ' '
    print(command, flush=True)
    os.system(command)
...
if loo_run:
    try:
        ...
        vals_loo = _calculate_loo_vals(sources, baseline_value, n_points, \
                                       eval_output_dir, train_run_command, eval_run_command, n_points_file)
...
for iter_counter in trange(max_iter, desc='Overall Shpley Value Calculation'):
    ...
    if tmc_run:
        # pdb.set_trace()
        error_ = error(mem_tmc, min_convergence_iter=save_every)
        print('=' * 50, flush=True)
        ...
        if error_< err :
            ...
            print('=' * 120, flush=True)
            mem_tmc, idxs_tmc = _tmc_shap (train_run_command, eval_run_command, eval_output_dir,\
                                 n_points_file, mem_tmc, idxs_tmc, random_score, mean_score, \
                                 n_points, save_every, seed, iter_counter, max_iter, tolerance, sources, print_step=1)
...
pdb.set_trace()
shapley_value_plots(directory, [vals_tmc, vals_loo], n_points, \
                    num_plot_markers=20, sources=sources, name=name+'_'+str(n_points))


performance_plots(directory, [vals_tmc, vals_loo], train_task_name, eval_task_name,\
                  train_run_command, eval_run_command, random_score, n_points, name=name+'_'+str(n_points), \
                  num_plot_markers=20, sources=None, rnd_iters=2, length=None)

performance_plots_adding(directory, [vals_tmc, vals_loo], train_task_name, eval_task_name,\
                         train_run_command, eval_run_command, random_score, n_points, name=name+'_'+str(n_points),\
                         num_plot_markers=20, sources=None, rnd_iters=2, length=None)
